...
This document describes the standard release process for listener-operator.
...
2. **Docker image** — uses `VERSION` directly:
   `quay.io/zncdatadev/listener-operator:<version>`
...
4. **Helm chart publish** — pushes to
   `quay.io/kubedoopcharts/listener-operator:<version>` (OCI registry)
...
```bash
gh api repos/zncdatadev/listener-operator/git/refs \
  -f ref=refs/heads/release-0.x \
  -f sha=$(gh api repos/zncdatadev/listener-operator/git/ref/heads/main --jq .object.sha)
```
...
- Docker image is available at
  `quay.io/zncdatadev/listener-operator:x.y.z-dev`
- Helm chart is available at
  `quay.io/kubedoopcharts/listener-operator:x.y.z-dev`
...
- **Release Image** — Builds and pushes multi-arch Docker image to
  `quay.io/zncdatadev/listener-operator:<version>`, and signs the image with
  Cosign
- **Release Chart** — Publishes the Helm chart to
  `quay.io/kubedoopcharts/listener-operator:<version>` (OCI registry) and
  updates the [kubedoop-helm-charts](https://github.com/zncdatadev/kubedoop-helm-charts)
  index

## Versioning Convention

listener-operator follows [Semantic Versioning](https://semver.org/):
